...
function Diagram({ mode, phase, meb, mpcShift }: { mode: CaseKey; phase: Phase; meb: number; mpcShift: number }) {
  ...
  const qAfter = phase === "initial" ? qp : mode === "parallel" ? (90 + mpcShift) / 2 : phase === "solves" ? qs : qstar;
  ...
  return (
    <svg className="diagram" viewBox="0 0 720 470" role="img" aria-label={`Economic diagram: ${cases[mode].title}`}>
      ...
      {phase === "initial" && <><polygon points={under} fill="url(#hatchX)" opacity=".8" /><text className="areaLabel" x={x((qp + qs) / 2)} y={y(mpc(qs)) - 30}>DWL X</text></>}
      ...
      {mode !== "parallel" && phase !== "initial" && <><line className="freeLine" x1={left} y1={base} x2={x(phase === "solves" ? qs : qstar)} y2={base} /><text className="freeLabel" x={left + 8} y={base - 10}>MPC′ = 0 (free)</text></>}
      ...
      {mode !== "parallel" && phase === "worsens" && <><polygon points={over} fill="url(#hatchY)" opacity=".78" /><line className="dwlBoundary" x1={x(qstar)} y1={y(msb(qstar))} x2={x(qstar)} y2={y(mpc(qstar))} /><text className="areaLabel y" x={x(overMidQ)} y={overMidY}>DWL Y</text><line className="guide" x1={x(qstar)} y1={base} x2={x(qstar)} y2={y(msb(qstar))} /><text className="qLabel star" x={x(qstar)} y={424}>Q*</text></>}
...
export default function Home() {
  ...
  const displayMeb = meb;
  const qs = Math.round((90 + displayMeb) / 2);
  const after = phase === "initial" ? 45 : phase === "solves" ? qs : mode === "parallel" ? Math.round((90 + mpcShift) / 2) : 100;
  const gap = after - qs;
  ...
  return <main>
    ...
    <section className="workspace">
      ...
      <aside className="panel"><span className="caseNo">THE ECONOMIC STORY</span><h3>{phase === "initial" ? "The free market underconsumes because MPB is below MSB. The market equilibrium is Qp, while the socially optimal quantity is Qs. The underconsumption from Qp to Qs creates welfare loss X." : phase === "solves" ? "Free provision lowers the marginal private cost faced by consumers. Consumption rises from Qp to Qs, eliminating welfare loss X and achieving allocative efficiency." : cases[mode].intro}</h3><div className="metric"><span>Marginal external benefit</span><strong>{displayMeb}</strong><input aria-label="Marginal external benefit" type="range" min="10" max="70" value={displayMeb} onChange={e => setMeb(+e.target.value)} /><small>Changing MEB moves the socially optimal quantity Qs.</small></div>{mode === "parallel" && phase !== "initial" && <div className="metric shiftControl"><span>Rightward shift of MPC</span><strong>{mpcShift}</strong><small>{phase === "solves" ? `The shift equals MEB (${displayMeb}), so Qa = Qs.` : `The shift exceeds MEB, so Qa > Qs.`}</small></div>}<div className="quantities"><div><span>Free market</span><b>Qp = 45</b></div><div><span>Social optimum</span><b>Qs = {qs}</b></div><div><span>{phase === "initial" ? "Current outcome" : "After policy"}</span><b>{phase === "initial" ? "Qp" : phase === "solves" ? "Qs" : after === 100 ? "Q*" : "Qa"} = {after}</b></div></div>
        <div className="logic"><h4>Chain of analysis</h4>{mode === "horizontal" && <ol><li>The free market consumes <b>Qp</b>, below the social optimum <b>Qs</b>, creating welfare loss <b>X</b>.</li><li><b>Solves:</b> free provision lowers MPC′ to zero and raises consumption from <b>Qp to Qs</b>, removing X.</li><li>Consumers do not stop at Qs; they continue until <b>MPB = MPC′ = 0 at Q*</b>.</li><li><b>Worsens:</b> because <b>Q* &gt; Qs</b>, society overconsumes and a new welfare loss <b>Y</b> is created.</li></ol>}{mode === "parallel" && <ol><li>The free market consumes <b>Qp</b>, below <b>Qs</b>, creating welfare loss <b>X</b>.</li><li><b>Solves:</b> shifting MPC rightwards raises consumption towards Qs and progressively removes X.</li><li>When the shift equals MEB ({displayMeb}), <b>Qa = Qs</b> and allocative efficiency is achieved.</li><li><b>Worsens:</b> a larger shift makes <b>Qa &gt; Qs</b>, causing overconsumption and a new welfare loss <b>Y</b>.</li><li>Move the slider below, to, and above {displayMeb} to compare the outcomes.</li></ol>}</div>
